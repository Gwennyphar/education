...
/** Funktion : BIN Datei umbenennen 
  ...
  */
  int iUmbennenen()
  {
      ...
      if( iJa_Nein() )
      {
          if( rename(DATEI, BACKUP) != 0 )/* wenn Datei nicht gefunden, dann abbrechen */
          {
              printf("\t\t   %s wurde nicht gefunden\t\t\r", DATEI);
              sleep(1000);
              iUmbennenen();      /* bleibe zur Fehlersuche in dieser Funktion*/
          }
          else         /* ... sonst wird hier die Datei erfolgreich umbenannt */
          {
              printf("\t\t   %s wird in %s ge\x84nder!\t\r", DATEI, BACKUP);
              sleep(1000);
              iWunschprogramm();             /* und springe zurueck ins Menue */
          }
      }
      else                             /* wenn Nein springe zurueck ins Menue */
      {
          iWunschprogramm();
      }
      return 0;
...
/** Funktion : BIN Datei loeschen
  ...
  */
  int iEntfernen()
  {
      ...
      if( iJa_Nein() )
      {
          if( remove(DATEI)!= 0 )/* wenn Datei nicht gefunden, dann abbrechen */
          {
              printf("\t\t   %s wurde nicht gefunden\t\t\r", DATEI);
              sleep(1000);
          ...
          else                       /* ...sonst Datei zum loeschen vormerken */
          {
              ...
              printf("\t\t     %s wird gel\x94scht!\t\t\r", DATEI);
              sleep(1000);
              iWunschprogramm();
          }
      }
