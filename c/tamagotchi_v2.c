...
void ASCII()
{
	...
	if(Tamagotchi.Koerpergewicht > 100.00 && Tamagotchi.Koerpergewicht < 550.00)
	{
		csr(7,55);
		printf("()__()");
		csr(8,55);
...
void Tier()
{
	...
	else if(Tamagotchi.Koerpergewicht > 65.00)
	{
		Tamagotchi.Koerpergewicht = 65.00;
	}
	if(Tamagotchi.Happyness < 10.00)
	{
		Tamagotchi.Happyness = 10.00;
	}
	...
	Tamagotchi.gegessenesSalz = 0.003;
	Anzeigen();
}
/*mit Salz behandeln*/
void Salz()
...
void Fuettern()
{
	...
	flush();
	if(Tamagotchi.futter < 0.00 || Tamagotchi.Hunger < 25.00)
	{
	}
	else
	{
		...
		if(Tamagotchi.Koerpergewicht > 450.00)
		{
			leben--;
		}
		clr();
...
void Trainieren()
{
	...
	else 
	{
		...
		if(Tamagotchi.Koerpergewicht < 00.00)
		{
			Tamagotchi.Koerpergewicht = 44.00;
		}
...
void Hygiene()
{
	...
	flush();
	if(Tamagotchi.reinigen < 0.00 || Tamagotchi.Happyness > 15.00)
	{
	}
	else
	{
		...
		{
			Tamagotchi.Happyness = 50.00;
		}
		if(Tamagotchi.Happyness == 100.00 && Tamagotchi.Hunger == 0.00)
		{
			leben++;
		}
		clr();
...
void Anzeigen()
{
	...
	while(1)
	{
		...
		csr(24,0);
        printf("     (c)2012 Nico Anders v.1.1");
        csr(12,39);
        Tamagotchi.Hunger = Tamagotchi.Hunger + 0.00005;
		Tamagotchi.Happyness = Tamagotchi.Happyness - 0.00002;
		if(Tamagotchi.Happyness < 8.00 || Tamagotchi.Hunger > 95.00 || Tamagotchi.Koerpergewicht < 46.00)
		{
			leben--;
		}
		if(Tamagotchi.Happyness < 0.00)
		...
		if(Tamagotchi.Hunger == 100.00 || Tamagotchi.Koerpergewicht < 45.00 || Tamagotchi.Happyness
		== 00.00 || leben == 0)
		{
			clr();
			printf("\n\n     T A M A G O T C H I  S A L Z - M O R D\n\n");
...
int iController()
{
	...
	do
	{
		...
		{
			Fuettern();
			if(Tamagotchi.Koerpergewicht > 550.00)
			{
				printf("     Ihr Tamagotchi ist gestorben");
				Tot();
				FP = fopen("lock", "w");
				fclose(FP);
				break;
