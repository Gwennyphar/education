...
/** Funtkion : Hilfeseite aufrufen
  ...
  */
  int iHilfe()
  {
	  ...
	  printf("\t\t\t      Zurueck zum Hauptmenue mit <Enter>");
	  getch() != 0x0d;                      /*Programm wartet auf Eingabetaste*/
	  return 0;
  }
...
/** Funtkion : Wunschtext Schreiben Klartext
  ...
  */
  int iSchreibenTxt()                      /*Funktion zum Schreiben der Txt File*/
  {
	  ...
	  getch() != 0x0d;                      /*Programm wartet auf Einagbetaste*/
	  return 0;
  }
...
/** Funtkion : Wusnchtext Lesen Klartext
  * Status   : in Abreit
  */
  ...
  */
  int iSchreibenBin()
  {
     ...
     for(iZaehler = 1; iZaehler <= ZEILEN; iZaehler++)
     {
		 ...
		 fflush(stdin);
		 fgets(sZeichen,LAENGE,stdin);
		 fwrite(sZeichen,LAENGE,1,FPWunsch);
	 }
	 fclose(FPWunsch);
	 printf("\n\n\t\t Wunscheingabe beendet!\r");
	 Sleep(1000);
...
/** Funtkion : Wusnchtext Lesen Binaerformat
  * Status   : in Abreit
  */
  int iLesenBin()
  {
	  ...
	  else
	  {
		  ...
		  do
		  {
			  for(iZaehler = 1; iZaehler <= ZEILEN; iZaehler++)
			  {
				  printf("\t\t %3i.Wunsch: %s",iZaehler, sZeichen);
				  fread(sZeichen,LAENGE,1,FPWunsch);
			  }     
          }while(!feof(FPWunsch));
	  }
	  fclose(FPWunsch);
